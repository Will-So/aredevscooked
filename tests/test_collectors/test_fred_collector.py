...
def test_collect_indeed_index(mock_get, collector, mock_fred_response):
    ...
    assert "q1_2023" in result["baselines"]
    # All 4 calls: latest + 30-day baseline + 1-year baseline + q1_2023 baseline
...
def test_collect_indeed_index_baselines_graceful_on_failure(
    ...
    mock_get.side_effect = [latest_resp, empty_resp, empty_resp, empty_resp]
    ...
    assert result["baselines"]["q1_2023"] is None


@patch("aredevscooked.collectors.fred_collector.requests.get")
def test_collect_total_indeed_index(mock_get, collector, mock_fred_response):
    mock_get.return_value = mock_fred_response(
        [{"date": "2026-03-01", "value": "95.50"}]
    )

    result = collector.collect_total_indeed_index()

    assert result["current_value"] == 95.50
    assert result["date"] == "2026-03-01"
    assert result["series_id"] == "IHLIDXUS"
    assert "baselines" in result
    assert "30_day" in result["baselines"]
    assert "1_year" in result["baselines"]
    assert "q1_2023" in result["baselines"]
...
@patch("aredevscooked.collectors.fred_collector.requests.get")
def test_collect_total_indeed_index_baselines_graceful_on_failure(
    mock_get, collector, mock_fred_response
):
    latest_resp = mock_fred_response([{"date": "2026-03-01", "value": "95.50"}])
    empty_resp = mock_fred_response([])
    mock_get.side_effect = [latest_resp, empty_resp, empty_resp, empty_resp]
